...
export class LandingPageComponent {
  ...
  ngOnInit(): void {
      ...
      As dusk descended, the city fell into abeyance, a temporary reprieve before chaos unleashed its \n fury once more.  
      ...
      In the end, amidst the labyrinth of deceit, truth prevailed, and Lane's resolve ameliorated the \n city's wounds, if only for a fleeting moment.`;
  ...
  tagSelectionWithText(tagStyle: string, tagText : string): void {
   ...
   // mark.id = this.selection.selId + "_" + divName+"_"+this.selection.index;
    ...
    markInput.addEventListener("keypress", (event: any) => {
      if(event.key === "Enter") {
        event.preventDefault();
       // alert("Enter in level  : "+ event.target.value);
    ...
    if(tagStyle === "dblline") {
      mark.style.borderBottom = "3px double black";
  ...
  private displayAnnotatedText ( event: any, selMap: any )
  {
    ...
    console.log(eleId, "eleIdRef", eleIdRef);
    ...
    if(eleRef) {
      eleRef.style.backgroundColor = "transparent";
    }
    if ( dupele )
    {
      ...
      var xPos = elePos?.x || 0;
      var yPos = elePos?.y || 0;
      var wid = elePos?.width || 0;
      var hei = elePos?.height || 0;
      if ( tagText.length > 0)
      {
        if(!this.playNow) {
          dupele.innerHTML = tagText.indexOf("data:image") < 0 ? `<span style="margin-left:${ document.getElementById( eleId )?.offsetLeft }px"> </span>` + tagText : `<img src="${ tagText }" style="margin-left:${ yPos - wid }px;max-width:100px;max-height:100px"/>`;
          dupele.style.color = "blue";
          dupele.style.fontSize = "14px";
          if ( eleParentPos )
          {
  ...
  checkAnswer(event:any, tagText: any, selMap: any) : boolean {
    console.log("tagText", tagText, this.level);
    var answer = tagText; //this.level === 2 ? tagText : (document.getElementById("answerId") as HTMLInputElement).value;
    var compareText = "";
    if(this.level === 2) {
  ...
  setLevel(level: number) {
    ...
    document.querySelectorAll("[data-annotated='yes']").forEach((ele:any) => {
      ...
      ele.querySelectorAll("input").forEach((ele1:any, index: any) => {
        ele1.style.display = "inline-block";
        ele1.className = "form-control inlineInput";
        ele1.style.border = "none";
        ele1.style.boxShadow = "none";
    ...
    document.querySelectorAll("[data-annotated='yes'] input").forEach((ele:any,index:any) => {
      if(index === 0) {
        ele.focus();
      }
    })
